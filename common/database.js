module.exports = (async () => {
  ...
  const promiseQuery = async (sql,vars,dbName) => {
    dbName = (dbName && dbName in DatabaseEnum)?dbName:DatabaseEnum.Main;
  ...
  const query = async (sql,vars,dbName) =>{
    ...
    try{
      ...
      try{
        ...
        console.error("z_database_error_log failed");
        console.log("e:",e);
      }
      console.log("database error dump: d.err",d.err);
      return d;  
    }
  ...
  const Main = { query: async (sql,vars,) => query(sql,vars,DatabaseEnum.Main) }
  const Files = { query: async (sql,vars,) => query(sql,vars,DatabaseEnum.Files) }
  ...
  return {
    ...
    query: query,
    getTransaction,
    Main:{query:Main.query, Config: DatabaseConfig.Main},
    Files:{query:Files.query, Config: DatabaseConfig.Files},
